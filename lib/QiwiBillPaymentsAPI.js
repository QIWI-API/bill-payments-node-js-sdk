...
module.exports = class QiwiBillPaymentsAPI {
    ...
    async _requestBuilder ({ url, method, body = null }) {
        ...
        const options = {
            hostname: 'qw-invoicing-api-stage.qiwi.com',
    ...
    checkNotificationSignature (signature, notificationBody, merchantSecret) {
        ...
        const convertedNotificationData = Object.keys(processedNotificationData).sort().map(
            (key) => processedNotificationData[key].toString()
        ).join(VALUE_SEPARATOR);

        const hash = crypto.createHmac(
            DEFAULT_ALGORITHM,
            merchantSecret
        ).update(convertedNotificationData).digest(DEFAULT_ENCODING);
     ...
     * @param      {Object}           params.customer                 The bill customer data
     * @return     {Promise<Object>|Error}  Return Promise with result
     */
