...
const publicKey = testConfig.merchantPublicKey;
...
describe("qiwi api v4", () => {
  try {
    ...
    it("creates payment form", () => {
      const testLink = `https://oplata.qiwi.com/create?publicKey=${publicKey}&amount=${parseFloat(amount).toFixed(
        2,
      )}&billId=${billId}&successUrl=http%3A%2F%2Ftest.ru%2F&customFields[apiClient]=${CLIENT_NAME}&customFields[apiClientVersion]=${packageJson.version}`;
    ...
    describe("util", () => {
      ...
      describe("signature check", () => {
        it("should return false on wrong signature", () => {
          assert.equal(false, qiwiApi.checkNotificationSignature(invalidSignatureFromNotificationServer, notificationData, merchantSecret));
        });

        it("should return true on valid signature", () => {
          assert.equal(true, qiwiApi.checkNotificationSignature(validSignatureFromNotificationServer, notificationData, merchantSecret));
        });
      });
    });
    ...
    describe("requests: ", () => {
      ...
      it("gets refund info", async () => {
        try {
          await qiwiApi.getRefundInfo(testConfig.billIdForGetRefundInfoTest, testConfig.billRefundIdForGetRefundInfoTest);
        } catch (err) {
      ...
      it("makes refund", async function () {
        try {
          await qiwiApi.refund(testConfig.billIdForGetRefundInfoTest, Date.now(), "0.01", "RUB");
        } catch (err) {
          const { serviceName, errorCode, description, userMessage } = err.error;
          assert.equal(serviceName, "invoicing-api");
          assert.equal(errorCode, "api.invoice.not.found");
